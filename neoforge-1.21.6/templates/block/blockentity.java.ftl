...
public class ${name}BlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer
    ...
    @Override public void loadAdditional(ValueInput input) {
        ...
        if(input.getInt("energyStorage") != null)
            energyStorage.deserialize(input);
        </#if>
        <#if data.isFluidTank>
        if (input.getBooleanOr("fluidTank",false))
        			fluidTank.deserialize(input);
        </#if>
        <#if data.sensitiveToVibration>
        if (input.contains("listener")) {
            VibrationSystem.Data.CODEC.parse(NbtOps.INSTANCE, input.getCompound("listener"))
    ...
    @Override public void saveAdditional(ValueOutput output) {
        ...
        VibrationSystem.Data.CODEC.encodeStart(NbtOps.INSTANCE, this.vibrationData)
                ...
                .ifPresent(listener -> output.put("listener", listener));
        </#if>
    }
