		...
		<#list tooltips as component>
			...
			<#if hasProcedure(component.displayCondition)>
				...
				if (mouseX > leftPos + ${x} && mouseX < leftPos + ${x + component.width} && mouseY > topPos + ${y} && mouseY < topPos + ${y + component.height}) {
					...
					if (hoverText != null) {
						guiGraphics.renderComponentHoverEffect(font, (net.minecraft.network.chat.Style) Arrays.stream(hoverText.split("\n")).map(Component::literal).collect(Collectors.toList()), mouseX, mouseY);
					}
					<#else>
                                                ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.${modid}.${registryname}.${component.getName()}"));
